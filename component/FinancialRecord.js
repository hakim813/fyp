...
export default function FinancialRecord() {
  ...
  const sortedData = [...data].sort(
    (a, b) => new Date(a.date) - new Date(b.date) // descending: newest first
  );
    ...
    onValue(postsRef, (snapshot) => {
      ...
      if (data) {
        ...
        const uniqueData = dataList
          .filter((item) => item.email === user.email) // First filter by user email
          .filter(
            (value, index, self) =>
                      ...
                      <View style={{ padding: 5 }}>
                        ...
                        {item.expense === 0 && item.income === 0 ? (
                          ...
                          <TouchableOpacity
                            onPress={() => {
                              setMonthChosen((prev) =>
                                prev === item.month ? null : item.month
                              );
                              console.log(
                                "FINANCIAL RECORD | Month chosen for detailed view : ",
                                monthChosen
                              );
                              setIsDetailedView((prev) =>
                                prev === true && monthChosen === item.month
                                  ? false
                                  : true
                              );
                            }}
                          ...
                          >
                            ...
                            {isDetailedView && item.month === monthChosen ? (
                              <View
                                style={{ borderTopWidth: 0.3, marginTop: 15 }}
                              >
                                <Text style={{ fontSize: 20, marginTop: 15 }}>
                                  Detailed transactions for {item.month}
                                </Text>
                                <FlatList
                                  // data={sortedData}
                                  data={sortedData.filter((item) => {
                                    const itemDate = new Date(item.date);
                                    const shortMonth = itemDate.toLocaleString(
                                      "default",
                                      { month: "short" }
                                    ); // e.g., "Apr"
                                    return shortMonth === monthChosen; // e.g., 3 for April
                                  })}
                                  keyExtractor={(i) => i.id}
                                  renderItem={({ item: i }) => {
                                    if (
